...
from atguigu.task.flow.links import FlowStepStaticLink, FlowStepConditionLink
from atguigu.task.flow.steps import FlowStep, StartFlowStep, EndFlowStep, ActionFlowStep, CollectFlowStep


class FlowExecutor:

    async def execute_flow(self, state: DialogueState, flow_list: FlowsList, action_runner: ActionRunner) -> list[
        BotMessage]:
        """
                  流程推进器
    ...
    def _advance_flow_step(self, state: DialogueState, step: FlowStep):
        ...
        state.current_activating_task().step_id = step_id  # 这里更新指向的是下一个步骤的ID

    def _select_step_id(self, state: DialogueState, step: FlowStep, FlowStepFallbackLink=None) -> str:
        """
        找一个step_id
        :param step:
        :return:
